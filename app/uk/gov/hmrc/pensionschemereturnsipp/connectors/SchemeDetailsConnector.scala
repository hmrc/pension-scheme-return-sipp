...
import uk.gov.hmrc.pensionschemereturnsipp.models.{SchemeDetails, SchemeId}
...
class SchemeDetailsConnectorImpl @Inject()(appConfig: AppConfig, http: HttpClient) extends SchemeDetailsConnector {
  ...
  override def details(
    ...
    http
      .GET[Option[SchemeDetails]](url("/pensions-scheme/scheme"))(
        implicitly,
  ...
  override def details(
    ...
    http
      .GET[Option[SchemeDetails]](url("/pensions-scheme/psp-scheme"))(
        implicitly,
        hc.withExtraHeaders(headers: _*),
        implicitly
      )
