...
class LandArmsLengthControllerSpec extends BaseSpec with TestValues with TestPayloads {
  ...
  private val controller = application.injector.instanceOf[LandArmsLengthController]

  "GET LandArmsLength" must {

    "return 200 with data" in {

      val response = LandOrConnectedPropertyResponse(
        transactions = List.empty
      )

      when(mockService.getLandArmsLength(any(), any(), any(), any())(any(), any()))
  ...
  "PUT LandArmsLength" must {
    ...
    "return 204 with data" in {
      ...
      val result = controller.put(Standard, Some("fbNumber"), None, None)(fakePutRequestWithBody(landArmsLengthPayload))
    ...
    "throws AuthorisationException when bearer token not supplied" in {
      ...
      val thrown = intercept[AuthorisationException] {
        await(
          controller.put(Standard, Some("fbNumber"), None, None)(fakePutRequestWithBody(landArmsLengthPayload))
        )
      }
      thrown.reason mustBe "Bearer token not supplied"
    }
